...
public static class GoblinLean
{
    ...
    /// <summary>x = 0 平面での鏡映。回転は軸を (x, -y, -z) にして角度はそのまま
    /// (クリップのミラー再生と同じ規約)。</summary>
    static Quaternion MirrorX(Quaternion q) { return new Quaternion(q.x, -q.y, -q.z, q.w); }
    ...
    // **SideP は SideN の鏡像**。2026-08-25 まで SideP は Boxing_Practice F105、SideN は
    // F60 という **無関係な 2 フレーム** から採っていたため、左右で別の姿勢になっていた。
    // F105 は胸が約 90 度ねじれたボクシングの構えで、「壺を右へ傾けたときだけ
    // お腹がねじれて見える」の正体がこれ (実測: 右傾きを左右反転して左傾きと比べると
    // Spine01 のロールが 138 度、Spine02 が 62 度ずれていた)。
    // SideP を足すときは必ず SideN の鏡像にすること。片側だけ差し替えないこと。
    ...
    public static readonly Quaternion SpineSideP = MirrorX(SpineSideN);
    ...
    public static readonly Quaternion Spine01SideP = MirrorX(Spine01SideN);
    ...
    public static readonly Quaternion Spine02SideP = MirrorX(Spine02SideN);
    ...
    public static readonly Quaternion NeckSideP = MirrorX(NeckSideN);
    ...
    public static readonly Quaternion HeadSideP = MirrorX(HeadSideN);
