...
public class FluidCore : MonoBehaviour, IPotionVolumeSource
{
    ...
    [Tooltip("滝の斜面衝突専用の反発係数(BoundsRestitutionより大きくして弾ませ、地形へ張り付いた見た目を防ぐ)。")]
    [Range(0f, 1f)] public float slopeRestitution = 0.35f;
    [Tooltip("滝の斜面衝突専用の摩擦(BoundsFrictionより小さくして、斜面を滑り落ちる勢いを殺さない)。")]
    [Range(0f, 1f)] public float slopeFriction = 0.03f;

    [Header("Waterfall Recycle (stage-branch addition, not in the Player-branch pot system)")]
    [Tooltip("Retired化した粒子をこのワールド空間の箱内にランダム再配置する。壺の RetiredPark(領域外に退避)を置き換えて、水源を作る。")]
    public Vector3 spawnBoxMin = new Vector3(-2.5f, 14.2f, -34.6f);
    public Vector3 spawnBoxSize = new Vector3(5f, 0.3f, 0.8f);
    [Tooltip("再配置直後に与える初期速度。下向きが基本。")]
    public Vector3 spawnVelocity = new Vector3(0f, -1.5f, 0f);

    [Tooltip("平らな GroundY 平面の代わりに、この高さプロファイルへ沿って衝突・反射させる(地形にぶつかりながら流れ落ちる見た目のため)。index 0 = slopeZStart、末尾 = slopeZEnd。未設定(null/空)なら従来通り平らな GroundY のみ。")]
    ...
    void ComputeScales()
    {
        ...
        // WATERFALL FIX (stage branch): the pot's box is ~1m^3, so a fixed 0.05 probe spacing was
        // cheap there. The waterfall's box covers the whole slope (tens of m^3) -- the same 0.05
        // spacing blows up to 1M+ boundary points (multi-second stall, and was intermittently
        // throwing in FluidBoundary.ComputeVolumes under this load). Box-mode InteriorVolumeWorld is
        // set directly from boxInnerSize regardless of point density (see FluidBoundary.BuildBox), so
        // this probe's only job is to populate that field cheaply -- scale the probe spacing to the
        // box size instead of using a size-oblivious constant.
        if (boundary.LocalPositions == null)
        {
            float probeSpacing = boundary.mode == FluidBoundary.Mode.Box
                ? Mathf.Max(0.05f, Mathf.Pow(boundary.boxInnerSize.x * boundary.boxInnerSize.y * boundary.boxInnerSize.z, 1f / 3f) * 0.05f)
                : 0.05f;
            boundary.Build(probeSpacing, probeSpacing * 2f);
        }
    ...
    void AllocateBuffers()
    {
        ...
        slopeProfile?.Release();
        if (slopeProfileHeights != null && slopeProfileHeights.Length >= 2)
        {
            slopeProfile = new GraphicsBuffer(GraphicsBuffer.Target.Structured, slopeProfileHeights.Length, sizeof(float));
            slopeProfile.SetData(slopeProfileHeights);
        }
        else
        {
            slopeProfile = new GraphicsBuffer(GraphicsBuffer.Target.Structured, 2, sizeof(float));
            slopeProfile.SetData(new float[] { groundY, groundY }); // flat fallback -- behaves exactly like the old single GroundY plane
        }
    }
    ...
    void Release()
    {
        ...
        slopeProfile?.Release(); slopeProfile = null;
    ...
    void BindAll(float dt, float lerpT)
    {
        ...
        Bind(kFinalize, ("PredictedPositions", predicted), ("Positions", positions), ("Velocities", velocities),
                        ...
                        ("Ages", ages), ("RetiredFlags", retiredFlags), ("SlopeProfileBuf", slopeProfile));
        ...
        // 滝用の追加(Player branch の壺システムには存在しない): FluidCore.compute の
        // Finalize は Retired(state==1) を RetiredPark に駐め置くが、滝ではその代わりに
        // この箱内へ再配置し、有限粒子数を循環させて「無限に流れ続けている」見た目を作る。
        ...
        int slopeCount = (slopeProfileHeights != null && slopeProfileHeights.Length >= 2) ? slopeProfileHeights.Length : 2;
        fluidCompute.SetInt("SlopeProfileCount", slopeCount);
        fluidCompute.SetFloat("SlopeZStart", (slopeProfileHeights != null && slopeProfileHeights.Length >= 2) ? slopeZStart : 0f);
        fluidCompute.SetFloat("SlopeZEnd", (slopeProfileHeights != null && slopeProfileHeights.Length >= 2) ? slopeZEnd : -1f);
