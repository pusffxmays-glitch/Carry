...
Shader "Custom/PotionLiquidSurface"
{
    Properties
    {
        _ShallowColor("Shallow (thin) Color", Color) = (0.25, 0.5, 1.0, 1)
        _BaseColor("Base Color", Color)              = (0.04, 0.12, 0.65, 1)
        _DeepColor("Deep Color", Color)              = (0.02, 0.04, 0.32, 1)
        _RimColor("Fresnel Rim Color", Color)        = (0.3, 0.65, 1.0, 1)
        ...
        // WATERFALL ADDITION (stage branch): the pot's version of this shader is fully
        // lighting-dependent (no true emission term), which is fine inside a lit interior but
        // reads as flat black in this forest scene's dark ambient lighting. _EmissionColor adds a
        // light-independent glow so the potion always reads as a luminous blue liquid instead of
        // a dark silhouette that only catches color from specular highlights.
        _EmissionColor("Emission (self-glow)", Color) = (0.05, 0.5, 3.0, 1)
        _EmissionIntensity("Emission Intensity", Range(0,4)) = 1.2
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                // 光源に依存しない自己発光。厚い部分でも暗い森の環境光だけに頼らず
                // 常に「光を帯びた液体」として読めるようにする。薄い縁は少し強めに発光させ、
                // 光が内部を透過しているような質感を出す。
                float3 emission = _EmissionColor.rgb * _EmissionIntensity * lerp(1.35, 0.85, thick01);
